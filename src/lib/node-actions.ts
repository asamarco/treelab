...
export async function copyNodesAction(
  ...
  // Clone-aware with two distinct behaviors:
  // 1. Top-level clones (same node selected multiple times): each gets a separate
  //    independent copy, since they all target the same destination parent and cannot
  //    form a meaningful clone relationship there.
  // 2. Descendant clones (same node encountered via different parents within the
  //    copied hierarchy): reuse the mapped ID and append additional parentIds/order
  //    entries, preserving the clone relationship in the pasted result.
  //
  ...
  const processHierarchy = (nodesToClone: TreeNode[], isDirectTopLevel: boolean) => {
    nodesToClone.forEach((node, index) => {
      ...
      if (alreadyMapped && isTopLevel) {
        // Top-level clone that was already encountered (as a descendant of
        // another selected node, or as a prior top-level item). Preserve the
        // clone relationship by adding the paste destination as an additional
        // parent to the existing node, rather than creating an independent copy.
        const existingNewNode = allNewNodesFlat.find((n: any) => n.id === idMap.get(node.id));
        if (existingNewNode && !existingNewNode.parentIds.includes(newParentIdForTopNodes)) {
          existingNewNode.parentIds.push(newParentIdForTopNodes);
          existingNewNode.order.push(newOrderForTopNodes + index);
        }
        // Children were already processed during the first encounter — no
  ...
  //  id: n.id, name: n.name, parentIds: n.parentIds, order: n.order
  //})), null, 2));
  //console.log('[copyNodesAction] idMap:', JSON.stringify(Array.from(idMap.entries())));
